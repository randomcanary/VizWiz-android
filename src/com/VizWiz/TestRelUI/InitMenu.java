...
public class InitMenu extends TextView implements Runnable {
	...
	//String selected;
	String sector; 
	private InitMenuShell parent;
	Thread gThread;
	String extraStuff = "";
	boolean recdQuery = false;
	String result = "";
	String resp_time, resp_resp;
	//take pic is u, history is L, checkresp is R, selpic is d

	public InitMenu(Context context) {
		super(context);
		parent = ((InitMenuShell) context);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (touched == MotionEvent.ACTION_DOWN) //draw menu
		{
	        ...
	        canvas.drawText("TAKE PICTURE", x_d, y_d-60,  paint); 
	        canvas.drawText("CHECK TODAYS ads", x_d+60, y_d,  paint);
	        canvas.drawText("SELECT PICTURE", x_d, y_d+60,  paint);
	        canvas.drawText("HISTORY", x_d-60, y_d,  paint);       
		}
		...
		{
		  if (sector.equalsIgnoreCase("TAKE PICTURE"))
		  {
			  Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
		        paint.setColor(Color.WHITE);
		        paint.setTextAlign(Paint.Align.CENTER);
		  ...
		  if (sector.equalsIgnoreCase("SELECT PICTURE"))
		  {
			  Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
		        paint.setColor(Color.WHITE);
		        paint.setTextAlign(Paint.Align.CENTER);
		  ...
		  if (sector.equalsIgnoreCase("HISTORY"))
		  {
			  Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
		        paint.setColor(Color.WHITE);
		        paint.setTextAlign(Paint.Align.CENTER);
		  ...
		  if (sector.equalsIgnoreCase("CHECK TODAYS ads"))
		  {
			  Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
		        paint.setColor(Color.WHITE);
		        paint.setTextAlign(Paint.Align.CENTER);
	...
	void launchMItem(String sector)
	{
		...
		else if (sector.equalsIgnoreCase("CHECK TODAYS ads"))
		{
			parent.titus.speak("Contact Walter Lasecki for all your candygram needs.", TextToSpeech.QUEUE_FLUSH, null);
		}
	...
	{
		if (parent.secondcall)
		{
			VWLaunchHTTP(parent.queryID);
			try {
				resp_resp = readJSONData(result, "text");
				resp_time = readJSONData(result, "time");
			} catch (Exception e) {
		...
		{
			extraStuff = " cannot execute. There are no queries. ";
			new Thread(this).start();	
		}
